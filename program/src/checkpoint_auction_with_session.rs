...
/// User checkpoints their auction rewards for a specific epoch (Fogo session)
pub fn process_checkpoint_auction_with_session<'a>(accounts: &'a [AccountInfo<'a>], data: &[u8]) -> ProgramResult {
    ...
    // Account order: signer, authority, program_signer, rig, miner, share, micro, well, oil_program
    let expected_len = 9;
    ...
    oil_api::extract_accounts!(accounts_iter, [s, a, ps, r, m, sh, mic, w, op]);
    let (signer_info, authority_info, program_signer_info, rig_info, miner_info, 
         share_info, micro_info, well_info, oil_program) = 
        (s, a, ps, r, m, sh, mic, w, op);
