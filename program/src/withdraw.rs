...
pub fn process_withdraw(accounts: &[AccountInfo<'_>], data: &[u8]) -> ProgramResult {
    ...
    // Load accounts.
    let clock = Clock::get()?;
    let [signer_info, authority_info, mint_info, recipient_info, stake_info, stake_tokens_info, pool_info, pool_tokens_info, miner_info, treasury_info, treasury_oil_info, system_program, token_program, associated_token_program] =
        accounts
    else {
    ...
    // Verify stake account matches the expected PDA for this stake_id
    ...
    if recipient_info.data_is_empty() {
        create_associated_token_account(
            signer_info, // payer (session payer) pays for ATA creation
